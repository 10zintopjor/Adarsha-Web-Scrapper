...
def writePage(page, opf_path,is_last):
...
def getwork(work, opf_path):
    ...
    while testUrl(work, i):
        ...
        for page in pages:
            writePage(page, opf_path,is_last)
        i += 10
...
def get_page_metadata(page,work_id,opf_path):
    ...
    for page in page_group:
        ...
        volume = int(re.search('(\d+?)-', page_start).group(1))
...
def get_span(page_start,page_end,opf_path):
    ...
    start_img = re.search("(\d+?-\d+?-\d+?[a-z]+)\d?",page_start).group(1).replace("line","")
    end_img = re.search("(\d+?-\d+?-\d+?[a-z]+)\d?",page_end).group(1).replace("line","")

    start_line = re.search("\d+?-\d+?-(\d+?)[a-z]+(\d?)",page_start).group(2)
    end_line = re.search("\d+?-\d+?-(\d+?)[a-z]+(\d?)",page_end).group(2)
...
def get_standard_sutra_format(value):
    res =  re.search("\d+?-\d+?-(\(.*\))?\d+?[a-z]+\d?",value).group(1)
...
def get_page_annotation(line,char_walker):
    ...
    img_num_first = re.search('(\d+?-\d+?)-\d+?[a-z]?', imgnum).group(1)
    img_num_last = get_img_num(re.search('\d+?-\d+?-(\d+[a-z]?)', imgnum).group(1))
    ...
    text_len = 0 if len(text)<=2 else len(text) -1 -2
    page_annotation = {
...
def parse_adarsha(ouput_path):
    ...
    #work = ["degekangyur", 2977724,"Kangyur/"]
    work = ['lhasakangyur', 2747736,"Kangyur/"]
    #work = ['jiangkangyur', 2561408,"Kangyur/"]

    ##Tengyur

    #work = ['degetengyur', 2843235,"Tengyur/"]

    #Works of Tibetan Masters

    #work = ['tsongkhapa', 1465969 , "Works_of_Tibetan_Masters/Geden/"] 

    #work = ['chodrapal', 2711704 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['choglenamgyal', 2973395 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['dolpopa', 2969222 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['gharungpa', 2715383 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['yeshegyatsho', 2728993 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['nyadbonkungapal', 2725129 , "Works_of_Tibetan_Masters/Jonang/"] Sutra Details not in standard format
    #work = ['thugsrjebrtsongrus', 2726404 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['lodropal', 2976581 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['matipanchen', 2720612 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['logrosgragspa', 2715915 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['yontenbzangpo', 2967211 , "Works_of_Tibetan_Masters/Jonang/"] Checked
    #work = ['sonamgragpa', 2977266 , "Works_of_Tibetan_Masters/Jonang/"] checked
    #work = ['tshalminpa', 2727252 , "Works_of_Tibetan_Masters/Jonang/"] Cheked
    #work = ['taranatha', 2730006 , "Works_of_Tibetan_Masters/Jonang/"] Checked

    #work = ['8thkarmapa', 1984307 , "Works_of_Tibetan_Masters/Kagyu/"] Checked
    #work = ['gampopa', 1464491 , "Works_of_Tibetan_Masters/Kagyu/"] Checked

    #work = ['mipam', 1489991 , "Works_of_Tibetan_Masters/Nyingma/"] Checked

    #work = ['gorampa', 1481195 , "Works_of_Tibetan_Masters/Sakya/"]  #Some Index span missing because of two different volume in one part


    #Texts of the Bön Tradition

    #work = ['bonpokangyur',2426116,"Texts_of_the_Bön_Tradition/"]

    opf_path = f"{ouput_path}/{work[2]}{work[0]}/{work[0]}.opf"
